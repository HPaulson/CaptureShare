...
const storage = multer.diskStorage({
    ...
    filename: function (req: any, file: {
        size: number,
        originalname: string
...
router.post('/', upload.array('file', 12), async (req: {
    headers: any;files: any[];body: any
}, res: {
    send: (arg0: any) => void;status: (arg0: number) => {
        (): any;
        new(): any;
        send: {
            (arg0: {
    ...
    if (!loggedUser) {
        ...
        await req.files.forEach(async (f: {
            filename: any;mimetype: any;size: any
        }) => {
            ...
            await uploads.create({
                ...
                new webcord({
                    ...
                    .send(config.webhook.events.fileUpload.replace(/{(.+?)}/g, (_: any, x: string) => replacers[x.trim()] || ''))
...
router.delete('/:id', async (req: {
    ...
    if (!loggedUser) {
        ...
        if (req.params.id === 'all') {
            ...
            docs.forEach(async (f: {
                _id: any
            }) => {
    
                let x;
                await uploads.findOne({_id: f._id}).then((file) => x = file)
                await uploads.deleteOne({
                    _id: f._id
                }).then(() => {
                    const replacers = {
                    ...
                    new webcord({
                        ...
                        .send(config.webhook.events.fileDelete.replace(/{(.+?)}/g, (_: any, x: string) => replacers[x.trim()] || ''))
                })
            ...
            let x;
            await uploads.findOne({_id: req.params.id}).then((f) => x = f)
            await uploads.deleteOne({
                    _id: req.params.id
                })
